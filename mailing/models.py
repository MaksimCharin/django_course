...
class MailingRecipient(models.Model):
    ...
    full_name = models.CharField(
        max_length=150, blank=True, null=True, verbose_name="Ф. И. О."
    )
...
class Mailing(models.Model):
    ...
    start_time = models.DateTimeField(
        verbose_name="Дата и время первой отправки", default=timezone.now
    )
...
class MailingAttempt(models.Model):
    ...
    def __str__(self):
        return (f'Попытка рассылки {self.mailing.pk if self.mailing else "N/A"} - Статус: '
                f'{self.get_status_display()} ({self.create_at.strftime("%Y-%m-%d %H:%M")})')
